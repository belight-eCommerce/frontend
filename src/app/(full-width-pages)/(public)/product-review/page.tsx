      ...
      <div className="flex-grow px-4 py-8 md:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
            ...
            {filteredReviews.length > 0 ? (
              filteredReviews.map((review, index) => (
                <div key={index} className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
                  ...
                  <p className="text-gray-700 my-3 text-sm italic">"{review.text}"</p>
                  <div className="flex justify-between items-center text-sm text-gray-500 pt-3 border-t border-gray-100">
                    <span>{review.date}</span>
                    <span className="font-semibold text-gray-800">{review.action}</span>
                  </div>
                </div>
              ))
            ) : (
          ...
          <div className="hidden md:block bg-white rounded-2xl shadow-sm w-full overflow-x-auto">
            <DataTable data={filteredReviews} columns={columns as any} />
          </div>
        </div>
      </div>
    </div>
